...
# The odd thing is, if we grow the value of n gradually, it wouldn't give
# the same stack overflow error anymore.
...
# Also for some reason running the test from terminal never finish 'include'ing
# this script file if I use the memoize package.( Comment out the first line and
# everything is fine).
...
function golomb2(n)
  ...
  tail = 2
    while maximum(keys(cache)) < n
      ...
      cache[maximum(keyChain) + cache[minimum(keyChain[keyChain .>= tail + 1])]] = tail + 1
      tail = tail + 1
    end
...
# Sadly this is still not fast enough ( I think), it takes 3 seconds to evaluate 1e6
# and 50 seconds to evaluate 1e7. It seems the time complexity is growing linearly.
...
# the speed achieved by this algorithm is still only remote from solving problem 341
# of Project Euler. Would you let me know what a faster ones would be like? Say a log
# time algorithm?
